		...
		{#if activeTab === 'account'}
			<Card>
				<SettingsRow title="Profile" description="Public student profile and visible account details.">
					<a href={`/profile/${data.user.id}`} class="btn-secondary">View profile</a>
				</SettingsRow>
			...
			<Card>
				<SettingsRow title="Theme" description="Choose light mode, dark mode, or system setting.">
					<form method="POST" action="?/settings">
						<select class="field-control" name="theme" onchange={(event) => (event.currentTarget as HTMLSelectElement).form?.requestSubmit()} aria-label="Theme">
		...
		{:else if activeTab === 'server' && data.user.role === 'admin'}
			<Card>
				<SettingsRow title="Server settings" description="Application-wide admin controls and storage reporting.">
					<a href="/admin/server-settings" class="btn-secondary">Open server settings</a>
				</SettingsRow>
			</Card>
		{:else if activeTab === 'security'}
			<Card className="max-w-2xl">
				<form class="form-stack" method="POST" action="?/password">
					<div>
						<h2>Password</h2>
						<p>Use at least 12 characters and avoid reused passwords.</p>
					</div>
					<div class="form-stack">
						<label class="form-row">
							<span>Current password</span>
							<input class="field-control" name="currentPassword" type="password" autocomplete="current-password" required />
						</label>
						<label class="form-row">
							<span>New password</span>
							<input class="field-control" name="newPassword" type="password" autocomplete="new-password" minlength="12" required />
						</label>
						<label class="check-field">
